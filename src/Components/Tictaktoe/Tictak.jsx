...
const Tictak = () => {
 ...
 let box_array = [box1,box2,box3,box4,box5,box6,box7,box8,box9]
  ...
  if(count % 2 === 0){
   e.target.innerHTML = `<img src="${crosspng}" alt="" />`
   data[num] = "x";
   setCount(++count)
  }
  else{
    e.target.innerHTML = `<img src="${circlePng}" alt="" />`
    data[num] = "o";
    setCount(++count)
  }
  checkWin()
...
if(data[0]===data[1] && data[1] === data[2] && data[2] !== ""){
  ...
  const won = (winner) =>{
    ...
    setLock(true);
    if(winner === "x"){
      titleRef.current.innerHTML = `congratulation cross <img src="${crosspng}" alt="" />`
    }
    else{
        titleRef.current.innerHTML = `congratulation circle <img src="${circlePng}" alt="" />`
    }
  }
  const restart = () =>{
    setLock(false);
